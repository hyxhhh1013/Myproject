...
export default function AiAssistantDemo() {
  ...
  return (
    <div className="min-h-full">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">AI 智能助手</h1>
        <p className="text-white/60">Online - GPT-4 Turbo</p>
      </div>

      {/* Chat Container */}
      <GlassCard className="flex flex-col h-[70vh]" rounded="2xl">
        {/* Chat Area */}
              ...
              {messages.map((msg) => (
                ...
                >
                  <div className={`flex max-w-[85%] md:max-w-[75%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start gap-3`}>
                    <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${msg.role === 'user' ? 'bg-gray-900 dark:bg-gray-700 text-white' : 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400'}`}>
                      {msg.role === 'user' ? <User size={16} /> : <Bot size={16} />}
                    </div>
                    
                    <div className={`p-4 rounded-2xl shadow-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white/5 dark:bg-gray-800 text-gray-800 dark:text-gray-200 border border-gray-100 dark:border-gray-700 rounded-tl-none'}`}>
          ...
          <div className="max-w-4xl mx-auto">
            <form onSubmit={handleSendMessage} className="relative">
              ...
              <input
                ...
                placeholder="输入消息..."
                className="w-full pl-24 pr-14 py-4 bg-transparent dark:bg-gray-700/50 border-transparent focus:bg-white/5 dark:focus:bg-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-xl outline-none transition-all text-gray-900 dark:text-white shadow-inner"
                disabled={isTyping}
              />
