...
## 功能特�?- **实时预览**: 所见即所�?- **Markdown 支持**: 
  - *斜体* �?**粗体**
  - 列表�?[链接](https://github.com)
  - 代码�?
\`\`\`javascript
...
> "简约是极致的复杂�? - 达·芬奇`,
  ...
  },
  {
    ...
    title: '项目灵感',
    content: `# 项目灵感
1. AI 驱动的待办事项应�?2. 具有毛玻璃特效的天气仪表�?3. 个人作品集网站`,
    date: '2023-10-25'
  }
];
...
const NotesDemo = () => {
  ...
  return (
    <div className="min-h-screen bg-white/5 dark:bg-gray-900 flex">
      {/* Sidebar */}
      <aside 
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-800">
           ...
           <div className="relative mb-4">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 bg-white/5 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
              />
           </div>
        ...
        <div className="flex-1 overflow-y-auto">
           {filteredNotes.map(note => (
             <div 
               ...
               className={`p-4 border-b border-gray-100 dark:border-gray-800 cursor-pointer group hover:bg-transparent dark:hover:bg-gray-800 transition-colors ${activeNoteId === note.id ? 'bg-white/5 dark:bg-gray-800 border-l-4 border-l-blue-600 shadow-sm' : 'border-l-4 border-l-transparent'}`}
             >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        {/* Mobile Header */}
        <header className="md:hidden bg-white/5 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between">
        ...
        {/* Toolbar */}
        <div className="hidden md:flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white/5 dark:bg-gray-900">
           ...
           <div className="flex gap-2">
              <button 
                ...
                className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'edit' ? 'bg-transparent dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-700'}`}
              >
                 编辑
              </button>
              <button 
                onClick={() => setActiveTab('preview')}
                className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'preview' ? 'bg-transparent dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-700'}`}
              >
                 预览
              </button>
           </div>
        ...
        <div className="flex-1 flex overflow-hidden relative">
          {/* Editor */}
          <div className={`flex-1 flex flex-col bg-white/5 dark:bg-gray-900 ${activeTab === 'preview' ? 'hidden md:flex' : 'flex'}`}>
             <input 
               type="text"
