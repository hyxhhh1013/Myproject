  ...
  const items = [
    {
      ...
      children: (
        ...
        >
          <Form.Item label="站点标题" name="siteTitle" rules={[{ required: true, message: '请输入站点标�? }]}>
            <Input placeholder="例如：我的个人作品集" />
          </Form.Item>
          
          <Form.Item label="关于�?(About Me)" name="aboutMe" help="这将显示在关于我页面">
            <TextArea rows={6} placeholder="介绍一下你自己..." />
          </Form.Item>

          <Form.Item label="SEO 关键�? name="seoKeywords" help="用逗号分隔，例如：摄影, 前端, 设计">
          ...
          <Form.Item label="ICP 备案�? name="icpCode">
            <Input placeholder="例如：京ICP备xxxxxxxx�? />
          </Form.Item>
      ...
      children: (
        ...
        >
          <Form.Item 
            ...
            rules={[{ required: true, message: '请输入当前密�? }]}
          >
            <Input.Password />
          </Form.Item>
