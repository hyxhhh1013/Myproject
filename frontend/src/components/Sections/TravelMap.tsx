...
export const TravelMap: React.FC = () => {
  ...
  // 状态管�?  const [cityData, setCityData] = useState<CityData[]>([]);
    ...
    const fetchCityData = async () => {
      try {
        ...
        const defaultData: CityData[] = [
          ...
          { id: 3, name: '武汉', province: '湖北', longitude: 114.31, latitude: 30.52, visitedAt: '2023-10-05', note: '家乡，珞珈山�?, photo: 'https://picsum.photos/seed/wuhan/300/200', rating: 5 },
          { id: 4, name: '北京', province: '北京', longitude: 116.40, latitude: 39.90, visitedAt: '2024-02-15', note: '故宫和长�?, photo: 'https://picsum.photos/seed/beijing/300/200', rating: 5 },
  ...
  // 初始化地�?  useEffect(() => {
    ...
    if (!mapRef.current || cityData.length === 0) {
      console.log('地图容器为空或城市数据为空，跳过初始�?);
      return;
    }
    ...
    const initECharts = async () => {
      try {
        ...
        // 直接使用echartsInstance.init()，不访问default属�?        const chart = echartsInstance.init(mapRef.current);
        chartRef.current = chart;
        console.log('ECharts实例初始化成�?);

        // 加载中国地图数据
        const loadMapData = async () => {
          try {
            console.log('开始加载地图数�?);
            // 尝试�?localStorage 获取缓存的地图数�?            const cachedMapData = localStorage.getItem('chinaMapData');
            console.log('localStorage中的地图数据:', cachedMapData ? '存在' : '不存�?);
            let mapData;

            if (cachedMapData) {
              console.log('使用缓存的地图数�?);
              ...
              console.log('地图数据加载成功，大�?', JSON.stringify(mapData).length);
              // 缓存地图数据
            ...
            // 注册地图
            console.log('开始注册地�?);
            // 直接使用echartsInstance.registerMap()，不访问default属�?            echartsInstance.registerMap(mapConfig.mapType, mapData);
            console.log('地图注册成功');
          ...
          <div className="bg-gray-800 rounded-xl overflow-hidden shadow-2xl max-w-2xl w-full mx-4">
            <div className="relative">
              ...
              <button
                ...
                className="absolute top-4 right-4 bg-white/5/20 backdrop-blur-sm w-8 h-8 rounded-full flex items-center justify-center text-white hover:bg-white/5/40 transition-colors"
              >
            ...
            <div className="p-6">
              ...
              <div className="flex justify-end">
                <button
                  onClick={closeCityInfo}
                  className="px-6 py-2 bg-white/5/10 rounded-full text-sm transition-all duration-300 hover:bg-white/5/20"
                >
                  关闭
                </button>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {cityData.map((city) => (
            <motion.div
              key={city.id}
              whileHover={{ y: -5 }}
              className="bg-white/5/5 backdrop-blur-sm rounded-lg p-4 hover:shadow-xl transition-all duration-300"
            >
