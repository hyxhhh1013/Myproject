...
const ProjectCard = ({ title, intro, stack, responsibilities, challenges, github, demoLink, DemoComponent, delay }: any) => {
  ...
  <motion.div
    ...
    className="bg-white/5 dark:bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group border border-gray-100 dark:border-gray-700 flex flex-col h-full"
  >
    {/* Interactive Demo Area */}
    <div className="h-64 bg-transparent dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 overflow-hidden relative">
    ...
    <div className="p-6 flex flex-col flex-grow">
      <div className="flex-grow">
        ...
        {/* Responsibilities */}
        <div className="mb-4">
          <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{t('project.responsibilities')}</h4>
        ...
        {challenges && (
          <div className="mb-4 bg-gray-50 dark:bg-gray-700/30 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
            <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">{t('project.challenges')}</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
              <span className="font-semibold text-gray-700 dark:text-gray-300">{t('project.problem')}：</span>{challenges.problem}<br/>
              <span className="font-semibold text-gray-700 dark:text-gray-300">{t('project.solution')}：</span>{challenges.solution}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-700 mt-2">
        {github ? (
          <a href={github} target="_blank" rel="noopener noreferrer" className="flex items-center text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors text-sm font-bold bg-transparent dark:bg-gray-700 px-3 py-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 z-20">
            <Github className="w-4 h-4 mr-2" /> {t('project.github')}
          </a>
        ) : (
...
export const Projects = () => {
  ...
  const localProjects = [
    ...
    },
    {
      ...
      responsibilities: [
        '使用 useEffect �?setInterval 实现精确的倒计时逻辑',
  ...
  return (
    <section id="projects" className="py-24 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      <div className="container mx-auto px-6">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-white mb-4">{t('projects.title')}</h2>
