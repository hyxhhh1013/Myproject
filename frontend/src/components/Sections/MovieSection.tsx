...
export const MovieSection = () => {
  ...
  // 当前显示的电影索�?  const [currentIndex, setCurrentIndex] = useState(0);
  ...
  const handleLike = async (movieId: number) => {
    try {
      ...
      // 更新本地状�?      const updatedMovies = movies.map(movie => {
        if (movie.id === movieId) {
  ...
  // 处理左滑�?  const handlePrev = () => {
    setCurrentIndex(prev => (prev === 0 ? movies.length - 1 : prev - 1));
  };
  
  // 处理右滑�?  const handleNext = () => {
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        ...
        <p className="widget-desc">一些深深打动我的电影，从救赎到荒诞到青�?/p>
      </div>
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="relative max-h-[80vh] flex items-center justify-center">
            ...
            {movies.map((movie, index) => {
              ...
              // 只显示当前海报前后的3张海�?              if (offset > 3) return null;
              ...
              return (
                <motion.div
                  ...
                  initial={{ opacity: 0 }}
                  animate={{
                    opacity: 0.08 / (offset + 1), // 进一步降低透明�?                    x: isLeft ? -40 * offset : 40 * offset, // 减小间距，增加堆叠密�?                    y: 8 * offset, // 减少垂直堆叠
              ...
              <div className="absolute inset-0 p-8 text-white flex flex-col justify-end">
                ...
                <div className="max-w-3xl">
                  ...
                  <motion.button
                    ...
                    className={`flex items-center gap-2 px-8 py-4 bg-white/5/10 backdrop-blur-sm rounded-full text-base font-medium transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105 group ${movies[currentIndex].isLiked ? 'text-red-400' : 'text-white'}`}
                  >
          ...
          <div className="flex justify-center gap-2 mt-6">
            {movies.map((_, index) => (
              <motion.button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-white/5 w-8' : 'bg-white/5/50'}`}
