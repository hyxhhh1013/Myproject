...
export function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-[#f5f5f7]">
      ...
      {/* Admin Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-white/5 border-r border-[#e5e5ea] 
      ...
      `}>
        <div className="h-full flex flex-col">
            ...
            <div className="p-6 flex items-center justify-between border-b border-gray-100">
                ...
                <button 
                  onClick={() => setIsSidebarOpen(false)}
                  className="md:hidden p-2 text-gray-500 hover:bg-transparent rounded-lg transition-colors"
                >
                  <X size={20} />
                </button>
            </div>
            ...
            <div className="p-4 flex-1 overflow-y-auto">
              ...
              <nav className="space-y-1">
                  ...
                  <AdminNavLink 
                    to="/admin/interests" 
                    current={location.pathname.startsWith('/admin/interests')} 
                    icon={<Heart size={18} />}
                  >
                      兴趣管理
                  </AdminNavLink>
            ...
            {/* <div className="p-4 border-t border-gray-100 bg-gray-50 md:bg-white/5 relative">...</div> */}
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-all duration-300">
        {/* Top Navigation Bar */}
        <header className="bg-white/5/80 backdrop-blur-md border-b border-[#e5e5ea] sticky top-0 z-30 px-4 py-3 md:px-6 flex items-center justify-between">
            <div className="flex flex-col">
                <div className="flex items-center">
                    <button 
                      onClick={() => setIsSidebarOpen(true)}
                      className="mr-3 md:hidden p-2 -ml-2 text-gray-600 hover:bg-transparent rounded-lg transition-colors"
                    >
                      <Menu size={24} />
                    </button>
            ...
            <div className="flex items-center gap-4">
               ...
               <div className="relative">
                   <button 
                    ...
                    className="flex items-center gap-3 hover:bg-transparent p-1.5 rounded-lg transition-colors"
                   >
                   ...
                   {isUserMenuOpen && (
                      <div className="absolute top-full right-0 mt-2 w-48 bg-white/5 border border-gray-100 rounded-xl shadow-lg overflow-hidden py-1 z-50 animate-in fade-in slide-in-from-top-2">
                        <button 
                          onClick={logout}
...
function getPageTitle(pathname: string): string {
  ...
  if (pathname.includes('/interests')) return '兴趣管理';
  return '管理后台';
}
