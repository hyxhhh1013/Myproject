...
var genericComponent = require('./generic-component.js');
...
ItemRender.getCollectionNode = function* getCollectionNode(props, context) {
    ...
    if (!textValue && !(context === null || context === void 0 ? void 0 : context.suppressTextValueWarning)) {
        exceptions.logWarning(`<Item>`, 'Non-plain text contents is unsupported by type-to-select for accessibility. Please add a `textValue` prop.');
    }
    yield {
        type: 'item',
        props,
...
const Item = /* @__PURE__ */ genericComponent.createGenericComponent('Item', ItemRender);
    ...
    yield {
        ...
        *childNodes() {
            if (typeof children === 'function') {
                if (!items) {
                    throw exceptions.createErrorWithCode('INVALID_COLLECTION_FACTORY_WITHOUT_ITEMS', `<Section>`, '`props.children` is a function but `props.items` is missing.');
                }
...
const Section = /* @__PURE__ */ genericComponent.createGenericComponent('Section', SectionRender);
...
function isValidCollectionChild(source, child) {
    if (React__default["default"].isValidElement(child) && !!child.type.getCollectionNode)
        return;
    throw exceptions.createErrorWithCode('INVALID_COLLECTION_INCOMPATIBLE_CHILDREN_DETECTED', source, 'Only children wrapped with <Item> or <Section> are allowed.');
}
/**
...
function validateCollection(props, source) {
    ...
    if (!items) {
        throw exceptions.createErrorWithCode('INVALID_COLLECTION_FACTORY_WITHOUT_ITEMS', source, '`props.children` is a function but `props.items` is missing.');
    }
    return (...args) => {
        var _a;
        ...
        if (key == null) {
            throw exceptions.createErrorWithCode('INVALID_COLLECTION_MISSING_KEY', source, 'No key found for item.');
        }
