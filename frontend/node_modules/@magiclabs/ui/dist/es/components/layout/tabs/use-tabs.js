...
function useTabs(config) {
    ...
    const tablist = (React.createElement(TabList, { orientation: orientation, label: tablistLabel, indicatorAttributes: indicatorAttributes, useAnimation: useAnimation }, tabs.map(({ id, label, icon }, i) => {
        const isSelected = activeTabID === tabIDs[i];
        return (React.createElement(Tab, { id: tabIDs[i], panelID: panelIDs[i], allTabIDs: tabIDs, index: i, activateTab: activateTab, selected: isSelected, orientation: orientation, size: size, useAnimation: useAnimation, ref: tabRefsCallback, icon: icon, key: id }, label));
    })));
    const tabpanel = tabs.map(({ id, content }, i) => {
        return (React.createElement(TabPanel, { id: panelIDs[i], tabID: tabIDs[i], selected: activeTabID === tabIDs[i], key: id }, content));
    });
