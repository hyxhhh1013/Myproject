...
router.post('/', authMiddleware_1.protect, projectController_1.upload.single('image'), async (req, res) => {
    await (0, projectController_1.createProject)(req, res);
    (0, cache_1.clearCache)('/api/projects');
});
router.put('/:id', authMiddleware_1.protect, projectController_1.upload.single('image'), async (req, res) => {
