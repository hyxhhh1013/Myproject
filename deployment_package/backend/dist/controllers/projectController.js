...
const createProject = async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            images = req.files.map((file) => `${CDN_BASE_URL}/uploads/demos/${path_1.default.basename(file.path)}`);
            imageUrl = images[0];
        }
...
const updateProject = async (req, res) => {
    try {
        ...
        // Handle new uploads
        if (req.files && req.files.length > 0) {
            const newImages = req.files.map((file) => `${CDN_BASE_URL}/uploads/demos/${path_1.default.basename(file.path)}`);
            finalImages = [...finalImages, ...newImages];
        }
        else if (!existingImages && !req.files) {
